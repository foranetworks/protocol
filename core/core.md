...
**Admittors**: Communities may optionally elect a trusted member to the role of admittor. Admittors are responsible for admitting new members into the community. They may also assign voting power, weighted against their own voting power, to members (if this is allowed by the community). They are appointed and replacable by governance.
...
Multiple admittors can grant voting powers to a single user. This way honest users can obtain more voting power in the community as the user is recognized by multiple admittors. When an admittor is kicked or replaced due to a governance proposal, only the weight attributed to this removed admittor is substracted by the users total voting power.
